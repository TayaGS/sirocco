...
calloc_import (int coord_type, int ndom)
{
  ...
  if (coord_type == SPHERICAL)
  {
    ...
    imported_model[ndom].heating = calloc (sizeof *imported_model[ndom].heating, NDIM_MAX);     // usr_heat
  }
  else if (coord_type == CYLIND || coord_type == RTHETA)
  {
    ...
    imported_model[ndom].heating = calloc (sizeof *imported_model[ndom].heating, NDIM_MAX2D);   // usr_heat
...
free_import (int coord_type, int ndom)
{
  if (coord_type == SPHERICAL)
  {
    ...
    free (imported_model[ndom].heating);        // usr_heat
  }
  else if (coord_type == CYLIND || coord_type == RTHETA)
  {
    free (imported_model[ndom].i);
